...
{
    public class JwtSrvc:Ijwt
    {
        // GENERAMOS EL TOKEN CON LA INFORMACIÓN DEL USUARIO 
        public string GenerarTokenJWT(Dictionary<string, string> ClaimsInfo,
        ...
        public string GenerarTokenJWT(Claim[] _Claims, int ID, string correoElectronico, IConfiguration _configuration, int horasExpiracion, int minutosExpiracion)
        {
            DateTime expiracion = System.DateTime.UtcNow.AddHours(horasExpiracion).AddMinutes(minutosExpiracion);
        ...
        public Dictionary<string, string> getInformacionToken(string JWT_Token)
        {
        ...
        public Dictionary<string, string> getInformacionToken(System.Security.Principal.IIdentity identidad)
        {
            Dictionary<string, string> info = new Dictionary<string, string>();
            if (identidad != null)
            {
        ...
        public string getClaimToken(System.Security.Principal.IIdentity identidad, string claimKey)
        {
            var infoToken = getInformacionToken(identidad);
            if (infoToken.ContainsKey(claimKey))
            {
                return infoToken[claimKey];
            }
            else
            {
                return "";
